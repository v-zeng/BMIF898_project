...
# df <- fread('KIRCNormsProcessedZeroNA.csv')
df <- fread('COADNormsProcessedZeroNA.csv')
...
(cbio <- cBioPortal())
ClinicalData <- clinicalData(cbio, "coadread_tcga") # coadread_tcga does not match DNAm data?
write.csv(ClinicalData,"cBioPortalclinicalCOAD.csv")

# filter KIRC clinical data to keep only matching patient IDs
kirpClinicalData <- fread('cBioPortalclinicalKIRP.csv')
KirpClinicalDataClean <- kirpClinicalData %>%  # can replace use of filter() by using subset()
  filter(kirpClinicalData$patientId %in% barcodeList) 

# KirpClinicalDataClean[1:5,1:5]

#===============================================================================

# create df with age and probes as columns ================================
df <- fread('KIRPNormsProcessedZeroNA.csv')
df <- subset(df,select=-(SD)) # drop last column, which is the SD column we added; or use 'allNormals_TCGABLCA_fall2021.csv' without this line
...
barcodeList <- colnames(dfSample_data)

# read in clinical data
dfClinical <- fread('cBioPortalclinicalKIRP.csv') # contains columns "patientID", "AGE"
...
# csv of above
write.csv(df_mergedClean,'KIRP_probesAge_merged.csv')
